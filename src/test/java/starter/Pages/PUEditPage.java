...
public class PUEditPage extends PageObject {
...
//        String getText = element.getText(); //return ""
    ...
    public void userNotEditable() {
        List<WebElement> elements = getDriver().findElements(By.className("wx-typo-sm warning--text"));
        Ensure.that(elements.size()>0);
        if(elements.size()>0) {
            elements.get(0).getText();
            String warning = getInnerText(elements.get(0));
            Ensure.that(warning.contains(" is not allowed to manage users") || warning.contains(" n'est pas autorisé à gérer les usagers"));
        }
    }
